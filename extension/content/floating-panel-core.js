...
// 尝试加载配置文件
try {
  console.log('[配置] 检测 EMAIL_CONFIG:', typeof EMAIL_CONFIG);
  console.log('[配置] 检测 TempMailClient:', typeof TempMailClient);
  
  if (typeof EMAIL_CONFIG !== 'undefined') {
    emailConfig = EMAIL_CONFIG;
    console.log('[模式] 配置已加载:', emailConfig.mode);
    console.log('[模式] tempMail配置:', JSON.stringify(emailConfig.tempMail));
    
    // 如果是临时邮箱模式，初始化客户端
    if (emailConfig.mode === 'temp-mail' && typeof TempMailClient !== 'undefined') {
      tempMailClient = new TempMailClient(emailConfig.tempMail);
      console.log('[临时邮箱] 客户端已初始化, provider:', tempMailClient.provider);
    } else {
      console.warn('[临时邮箱] 客户端未初始化! mode:', emailConfig.mode, 'TempMailClient:', typeof TempMailClient);
    }
  } else {
...
// 导出初始化函数供floating-ball.js调用
window.initFloatingPanel = initFloatingPanel;
    ...
    if (restored) {
      ...
      if (stateMachine.shouldAutoRestore() && metadata.email) {
        ...
        if (state === RegistrationStateMachine.STATES.WAITING_VERIFICATION) {
          ...
          // 显示停止按钮
          document.getElementById('floating-start-btn').classList.add('hidden');
...
function setupEventListeners() {
  document.getElementById('floating-start-btn').addEventListener('click', startRegistration);
  document.getElementById('floating-stop-btn').addEventListener('click', stopMonitoring);
  document.getElementById('floating-reset-btn').addEventListener('click', resetRegistration);
  document.getElementById('floating-accounts-btn').addEventListener('click', viewAccounts);
  document.getElementById('floating-stats-btn').addEventListener('click', viewStats);
  
  // 超级智能大脑按钮
  document.getElementById('floating-brain-btn').addEventListener('click', openSuperBrain);
  
  // 复制按钮（使用事件委托）
  document.addEventListener('click', (e) => {
    if (e.target.id === 'floating-copy-email-btn') {
      copyToClipboard(currentAccount?.email, '邮箱');
    } else if (e.target.id === 'floating-copy-password-btn') {
...
async function startRegistration() {
  ...
  // 在悬浮球模式下，直接检查当前页面URL（不需要chrome.tabs API）
  const currentUrl = window.location.href;
  
  if (!isWindsurfRegistrationPage(currentUrl)) {
  ...
  if (isContinue) {
    // 继续模式：直接调用fillForm函数（悬浮球和content-script在同一上下文）
    log('📝 使用现有账号继续填充表单');
    
    try {
      // 直接调用content-script.js中的handleFillForm函数
      const response = await handleFillForm(currentAccount);
      ...
      console.error('[FloatingPanel] 填充表单错误:', error);
      stateMachine.transition(RegistrationStateMachine.STATES.ERROR, {
        error: error.message
      });
      await stateMachine.saveToStorage();
      log('❌ 填充失败: ' + error.message, 'error');
      resetUI();
    }
  } else {
    // 新建模式：根据配置选择账号生成方式
    ...
    if (emailConfig && emailConfig.mode === 'temp-mail' && tempMailClient) {
      ...
      try {
        ...
        // 7. 直接调用handleFillForm填充表单（悬浮球模式）
        try {
          const fillResponse = await handleFillForm(accountData);
          ...
          if (fillResponse && fillResponse.success) {
            ...
            log('❌ 填充失败: ' + (fillResponse?.error || '未知错误'), 'error');
            resetUI();
          }
        } catch (fillError) {
    ...
    if (response && response.success) {
      ...
      if (response.email) {
        ...
        // 直接调用handleFillForm填充表单（悬浮球模式）
        log('📝 开始填充表单...');
        try {
          const fillResult = await handleFillForm(response);
          if (fillResult && fillResult.success) {
            ...
            log('❌ 填充失败: ' + (fillResult?.error || '未知错误'), 'error');
            resetUI();
          }
        } catch (fillError) {
          log('❌ 填充失败: ' + fillError.message, 'error');
...
async function startTempMailMonitoring(email) {
  console.log('[startTempMailMonitoring] 开始，邮箱:', email);
  console.log('[startTempMailMonitoring] isMonitoring:', isMonitoring);
  console.log('[startTempMailMonitoring] tempMailClient:', tempMailClient);
  ...
  if (!tempMailClient) {
    ...
    console.error('[startTempMailMonitoring] tempMailClient is null!');
    console.log('[startTempMailMonitoring] emailConfig:', emailConfig);
    console.log('[startTempMailMonitoring] TempMailClient type:', typeof TempMailClient);
    return;
  }
  
  console.log('[startTempMailMonitoring] tempMailClient.provider:', tempMailClient.provider);
  console.log('[startTempMailMonitoring] tempMailClient.currentEmail:', tempMailClient.currentEmail);
  console.log('[startTempMailMonitoring] tempMailClient.currentToken:', tempMailClient.currentToken ? '已设置' : '未设置');
  
  isMonitoring = true;
  updateStatus('running', '监听验证码中...');
  log('📧 开始监听临时邮箱: ' + email);
  log('⏳ 预计等待时间: 最多5分钟');
  log('💡 请在页面上完成注册表单提交，验证码邮件将自动获取');
  ...
  try {
    ...
    console.log('[FloatingPanel] 验证码获取结果:', result);
    
    if (result.success && result.code) {
      log(`🎉 自动获取到验证码: ${result.code}`, 'success');
      displayVerificationCode(result.code);
      updateStatus('success', '验证码已获取');
      
      // 自动填充验证码到页面
      try {
        fillVerificationCode(result.code);
        log('✅ 验证码已自动填充到页面');
      } catch (fillError) {
        log('⚠️ 自动填充失败，请手动输入验证码', 'warning');
      }
...
function stopRealtimeMonitoring() {
  ...
  document.getElementById('floating-start-btn').classList.remove('hidden');
...
async function resetRegistration() {
  ...
  // 清空日志
  document.getElementById('floating-logs').innerHTML = '';
...
function displayVerificationCode(code) {
  ...
  // 显示账号信息区域
  const accountInfoDiv = document.getElementById('floating-current-account');
  accountInfoDiv.classList.remove('hidden');
  
  // 填充账号信息
  document.getElementById('floating-account-email').textContent = currentAccount.email;
  document.getElementById('floating-account-password').textContent = currentAccount.password;
  document.getElementById('floating-account-username').textContent = currentAccount.username;
  
  // 检查是否已存在验证码字段，避免重复创建
  let codeField = document.getElementById('floating-code-field-container');
  
  if (!codeField) {
    // 首次创建验证码字段
    codeField = document.createElement('div');
    codeField.id = 'floating-code-field-container';
    codeField.className = 'field';
    codeField.innerHTML = `
      <label>验证码:</label>
      <div style="display: flex; align-items: center; gap: 8px;">
        <span id="floating-verification-code" style="font-weight: bold; color: #10b981; font-size: 16px;">${code}</span>
        <button id="floating-copy-code-btn" class="btn btn-primary" style="padding: 4px 12px; font-size: 12px;">复制</button>
      </div>
    `;
    ...
    setTimeout(() => {
      const copyBtn = document.getElementById('floating-copy-code-btn');
      if (copyBtn) {
    ...
    // 更新已存在的验证码
    const codeSpan = document.getElementById('floating-verification-code');
    if (codeSpan) {
...
function updateStatus(status, text) {
  const indicator = document.getElementById('floating-status-indicator');
...
function displayAccountInfo(account) {
  ...
  const accountInfoDiv = document.getElementById('floating-current-account');
  accountInfoDiv.classList.remove('hidden');
  document.getElementById('floating-account-email').textContent = account.email || 'N/A';
  document.getElementById('floating-account-password').textContent = account.password || 'N/A';
  document.getElementById('floating-account-username').textContent = account.username || 'N/A';
  const sidEl = document.getElementById('floating-account-session');
  if (sidEl) {
...
function updateButtonState(mode) {
  const startBtn = document.getElementById('floating-start-btn');
...
function log(message, type = 'info') {
  const logs = document.getElementById('floating-logs');
...
async function openSuperBrain() {
  ...
  // 显示加载提示
  const container = document.getElementById('floating-brain-container');
  container.innerHTML = `
...
function viewStats() {
  log('📊 打开统计分析页面');
  // 在悬浮球模式下，通过background script打开新窗口
  chrome.runtime.sendMessage({
    action: 'openWindow',
    url: 'popup/stats.html',
    width: 520,
    height: 650
  });
}
